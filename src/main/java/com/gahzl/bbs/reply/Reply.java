...
public class Reply extends Model<Reply> {
    ...
    public Page<Reply> page(int pageNumber, int pageSize) {
        return super.paginate(pageNumber, pageSize, new String[]{"select r.*, t.title, u.nickname ", "from reply r left join topic t on r.tid = t.id left join user u on r.author_id = u.id order by r.in_time desc"});
    }
