...
import com.gahzl.bbs.utils.AgentUtil;
import com.gahzl.bbs.utils.DateUtil;
import com.gahzl.bbs.utils.EmailSender;
...
public class IndexController extends BaseController {
    ...
    public void index() {
        ...
        Integer pageSize = PropKit.use("config.properties").getInt("page_size");
        Page<Topic> page = Topic.me.paginate(getParaToInt("p", 1),getParaToInt("size", pageSize), tab, q, 1, l);
            ...
            if (type.equalsIgnoreCase(Constants.ValiCodeType.FORGET_PWD)) {
                ...
                if (user == null) {
                    ...
                    code.set("code", valicode)
                            ...
                            .set("target", email)
                            .save();
                    EmailSender.sendMail(PropKit.use("config.properties").get("site_title")+"－找回密码验证码", new String[]{email}, "您找回密码的验证码是：" + valicode + "\r\n" + "该验证码只能使用一次，并且有效期仅30分钟。");
                    success();
                }
                ...
                if (user != null) {
                    ...
                    code.set("code", valicode)
                            ...
                            .set("target", email)
                            .save();
                    EmailSender.sendMail(PropKit.use("config.properties").get("site_title")+"－注册账户验证码", new String[]{email}, "您注册账户的验证码是：" + valicode + "\r\n" + "该验证码只能使用一次，并且有效期仅30分钟。");
                    success();
                }
            }
        }
