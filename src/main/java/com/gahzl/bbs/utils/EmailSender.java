...
public class EmailSender {
    ...
    public void send(String subject, String[] mailTo, String mailBody) throws Exception {
        try {
            ...
            props.put("mail.smtp.host", PropKit.get("email.host"));
            props.put("mail.smtp.auth", "true");
            Session session = Session.getDefaultInstance(props, auth);
            // 设置session,和邮件服务器进行通讯。
            MimeMessage message = new MimeMessage(session);
            // message.setContent("foobar, "application/x-foobar"); // 设置邮件格式
            message.setSubject(subject == null?PropKit.get("email.default_subject"):subject); // 设置邮件主题
            message.setText(mailBody); // 设置邮件正文
            // message.setHeader(mail_head_name, mail_head_value); // 设置邮件标题
            message.setSentDate(new Date()); // 设置邮件发送日期
            Address address = new InternetAddress(mail_from, PropKit.get("email.personal_name"));
    ...
    public static void main(String[] args) {
        sendMail("test", new String[]{"623837472@qq.com"}, "测试邮件内容");
    }
